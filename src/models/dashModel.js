...
function buscarPreferenciasUsuario(idCliente) {
    ...
    var instrucaoSql = `
    SELECT p.categoria, p.tom, p.estilo FROM preferencia pr 
    JOIN produto p ON pr.fkProduto = p.idProduto 
    WHERE pr.fkCliente = ${idCliente}
    ORDER BY pr.idPreferencia DESC
    LIMIT 1;`;
